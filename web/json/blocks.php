<?php
	...
	if(isset($_GET['algo']))
	{
		...
		$query = "SELECT height,hash,pow_algo,difficulty,time,elapsed,algo_elapsed,auxpow FROM tblnetworkblocks WHERE pow_algo=$algo ORDER BY height DESC LIMIT 0,$count";
	}
	else
	{
		$query = "SELECT height,hash,pow_algo,difficulty,time,elapsed,algo_elapsed,auxpow FROM tblnetworkblocks ORDER BY height DESC LIMIT 0,$count";
	}

	function get_algoname($id, $time)
	{
		switch ($id)
		{
			case 0:
	...
	while($row = $result->fetch_row())
	{
		...
		$info->Blocks[$i]->Algorithm = get_algoname($row[2],$row[4]);
		...
		$info->Blocks[$i]->AlgoElapsedProp = sprintf("%+d",round(($row[6]-300)/1.5,0));
		$info->Blocks[$i]->AuxPoW = ($row[7]==1);
		
		if($info->Blocks[$i]->Height==0)
		{
			$info->Blocks[$i]->Algorithm = 'Genesis';
			$info->Blocks[$i]->Hash = 'Genesis';
			$info->Blocks[$i]->AlgoElapsed = '0';
			$info->Blocks[$i]->AlgoElapsedProp = '0';
			$info->Blocks[$i]->Elapsed = 0;
		}
		$i++;
	}
	...
	function get_stats($dblink, $period)
	{
		...
		$query = "SELECT COUNT(height) FROM tblnetworkblocks WHERE time >= UNIX_TIMESTAMP(DATE_ADD(NOW(),INTERVAL -$period))";
		$result = $dblink->query($query) OR dberror($dblink,$query);
		...
		$query = "SELECT pow_algo,COUNT(height),AVG(difficulty) FROM tblnetworkblocks WHERE time >= UNIX_TIMESTAMP(DATE_ADD(NOW(),INTERVAL -$period)) GROUP BY pow_algo";
		$result = $dblink->query($query) OR dberror($dblink,$query);
		while($row = $result->fetch_row())
		{
			$id = $row[0];
			...
			$data->Algo[$id]->Hashrate = scale_rate($row[2] * pow(2,32) / 300);
		}
